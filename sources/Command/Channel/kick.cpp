...
int	Server::kick_user(Channel &channel, User &user_from, User &user_to, std::vector<std::string> params)
{
	...
	{
		this->add_client("441 KICK " + user_to.getNick() + " " + channel.getName() + " :They aren't on that channel", user_from);
		return (0);
	}
	...
	if (params.size() > 2 && params[2].size() != 1)
		this->add_client("KICK " + params[0] + " " + user_to.getNick() + params[2], user_from, user_to);
	else
		this->add_client("KICK " + params[0] + " " + user_to.getNick() + " :has been kicked", user_from, user_to);
	return (1);
}
...
int		Server::cmd_kick(std::vector<std::string> params, User &user)
{
	...
	if (user.getOp() == false)
	{
		this->add_client("481 :Permission Denied- You're not an IRC operator", user);
		return (0);
	}
	if (params.size() < 2)
	{
		this->add_client("461 KICK :Not enough parameters", user);
		return (0);
	}
	std::map<std::string, Channel>::iterator chan = this->_channels.find(params[0]);
	if (chan == this->_channels.end())
	{
		this->add_client("403 KICK :No such channel", user);
		return (0);
	}
	if (chan->second.isUserInChannel(user) == false)
	{
		this->add_client("442 KICK :You're not on that channel", user);
		return (0);
	}
	...
	for (std::vector<std::string>::iterator it = to_kick.begin(); it != to_kick.end(); it++)
	{
		...
		{
			this->add_client("441 KICK " + *it + " " + chan->second.getName() + " :They aren't on that channel", user);
			ok = false;
			continue;
		}
